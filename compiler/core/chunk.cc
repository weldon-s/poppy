...
Line Chunk::push_chunk() const {
    ...
    return movi(Register::scratch, size) + Line(new Instruction(Instruction("sub sp, sp, x9") + "str x9, [sp]"));
}
...
Line Chunk::read_variable(const Register& reg, const Variable& variable) const {
    ...
    return Line(new Instruction(Instruction(std::format("ldr {}, [sp, #{}]", reg, offsets.at(variable)))));
}

Line Chunk::write_variable(const Variable& variable, const Register& reg) const {
    // write the variable to the stack
    // precondition: sp points to top of stack
    return Line(new Instruction(Instruction(std::format("str {}, [sp, #{}]", reg, offsets.at(variable)))));
}